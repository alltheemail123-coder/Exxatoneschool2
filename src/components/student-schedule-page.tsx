...
import { DataTable, type ColumnConfig, type PaginationInfo } from "./data-table"
import type { ActiveFilter, FilterConfig } from "./filter-bar"
...
export function StudentSchedulePage({ onItemClick }: StudentSchedulePageProps) {
  ...
  // ── React 18: useDeferredValue for search ──────────────────────────────
  // Keeps the search input responsive while the table re-filters in the background.
  // The input updates immediately (searchQuery), but the expensive filtering
  // uses the deferred value, allowing React to interrupt and keep the UI responsive.
  const deferredSearchQuery = React.useDeferredValue(searchQuery)

  // ── React 18: useTransition for tab switching ──────────────────────────
  // Tab changes trigger column reconfiguration, data re-filtering, and pagination reset.
  // Wrapping in startTransition lets React keep the tab bar responsive
  // while the table content catches up in the background.
  ...
  // Update columns and reset pagination when tab changes
  React.useEffect(() => {
   ...
   *
   * Primary pages should display 4 key metrics that are:
   *   1. Aggregate – a high-level count showing overall volume
   *   2. Urgency  – items needing immediate attention (time-sensitive)
   *   3. Risk     – items that may fail without intervention
   *   4. Quality  – a rate or percentage reflecting overall health
   *
   * Each metric should be clickable, navigating to or filtering
   * the relevant tab/data subset below.
   */
  const computedMetrics = React.useMemo(() => {
    ...
    return [
      createSimpleMetricData("Total Placements", String(totalPlacements), {
        trend: "up",
        trendValue: "+12",
      }),
  ...
  // ── Filter configs derived from actual student data ────────────────────
    ...
    () => [
      {
        ...
        icon: Shield,
        options: Array.from(new Set(studentScheduleData.map((s) => s.readinessStatus))).sort(),
      },
      {
        key: "discipline",
        label: "Discipline",
        icon: GraduationCap,
        options: Array.from(new Set(studentScheduleData.map((s) => s.discipline))).sort(),
      },
      {
        key: "specialization",
        label: "Specialization",
        icon: Briefcase,
        options: Array.from(new Set(studentScheduleData.map((s) => s.specialization))).sort(),
      },
      {
        key: "siteName",
        label: "Site",
        icon: Building2,
        options: Array.from(new Set(studentScheduleData.map((s) => s.siteName))).sort(),
      },
      {
        key: "location",
        label: "Location",
        icon: MapPin,
        options: Array.from(new Set(studentScheduleData.map((s) => s.location))).sort(),
      },
      {
        key: "duration",
        label: "Duration",
        icon: Clock,
        options: Array.from(new Set(studentScheduleData.map((s) => s.duration))).sort(),
      },
    ],
    [studentScheduleData]
  )
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (activeMetricFilter) {
      if (activeMetricFilter === "starting-this-week") {
        data = data.filter((item) => item.daysUntilStart <= 7)
      } else if (activeMetricFilter === "compliance-alerts") {
        data = data.filter((item) => item.compliancePercent < 85)
      } else if (activeMetricFilter === "at-risk") {
        data = data.filter((item) => item.progressPercent < 50 || item.lastCheckin === "7+ days ago")
      }
    }

    // Active filters from FilterBar
    ...
    // Default sort for upcoming
    if (activeTab === "upcoming") {
      data = [...data].sort((a, b) => a.daysUntilStart - b.daysUntilStart)
    }

    return data
  }, [studentScheduleData, activeTab, deferredSearchQuery, activeMetricFilter, activeFilters])

  // Update view counts based on actual data
  React.useEffect(() => {
    ...
    setViews((prev) =>
      prev.map((v) => ({
        ...v,
        count: String(counts[v.id] ?? v.count),
      }))
    )
  }, [studentScheduleData])
  ...
  // Metric card click handlers — wrapped in startTransition for responsive UI
  const handleMetricClick = React.useCallback(
    (index: number) => {
      startTabTransition(() => {
        if (index === 0) {
          // Total Placements — show all on upcoming tab
          ...
          // Starting This Week
          setActiveTab("upcoming")
          setActiveMetricFilter("starting-this-week")
          setCurrentPage(1)
        } else if (index === 2) {
          // Compliance Alerts
          setActiveTab("upcoming")
          setActiveMetricFilter("compliance-alerts")
          setCurrentPage(1)
        } else if (index === 3) {
          // Avg Compliance — show completed for historical data
  ...
  // Clear metric filter when tab changes manually
  React.useEffect(() => {
      ...
      switch (column.key) {
          ...
          return (
            <div className="flex items-start gap-2 w-full overflow-hidden">
              <div className="flex-1 space-y-1 min-w-0">
                <div
                  className={cn(
                    "data-table-clickable font-medium text-base",
                    !column.wrapText && "line-clamp-1"
                  )}
                  onClick={() => onItemClick?.(item.id, item.studentName, item.siteName)}
                >
                  {item.studentName}
                </div>
                <div className={cn(
                  "text-xs text-foreground",
                  !column.wrapText && "truncate"
                )}>{item.studentEmail}</div>
              </div>
              {activeTab === "upcoming" && (
                <TooltipProvider>
                    ...
                    {item.hasNewActivity && (
                      <Tooltip>
                        ...
                        <TooltipContent side="top" className="bg-popover text-popover-foreground border border-border">
                          <p className="text-xs">New activity on this schedule</p>
                        </TooltipContent>
                      </Tooltip>
                    )}
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="relative group cursor-help">
                            ...
                            <span className="sr-only">Has notification</span>
                          </div>
                        </TooltipTrigger>
                        <TooltipContent side="top" className="bg-popover text-popover-foreground border border-border">
                          <p className="text-xs">Pending notifications or messages</p>
                        </TooltipContent>
                      </Tooltip>
                    )}
                  </div>
          ...
          return (
            <div className="space-y-1 w-full overflow-hidden">
              ...
              <div className="text-xs text-foreground truncate">
                {item.courseName} &bull; {item.availabilityName}
              </div>
        ...
        case "siteName":
          return (
            <div className="space-y-1 w-full overflow-hidden">
              <div
                className="truncate font-medium text-base data-table-clickable"
                onClick={() => console.log("Navigate to site:", item.siteName)}
              >
                {item.siteName}
              </div>
        ...
        case "startDate":
          return (
            <div className="space-y-1">
              <div className="font-medium text-base">{item.startDate}</div>
        ...
        case "endDate":
          return (
            <div className="space-y-1">
              <div className="font-medium text-base">{item.endDate}</div>
              <div className="text-xs text-muted-foreground">{item.duration}</div>
            </div>
          )
            ...
            <Badge
              ...
              className={
                item.readinessStatus === "Ready"
                  ...
                  : item.readinessStatus === "Action Required"
          ...
          return (
            <div className="space-y-1">
              ...
              <div className="text-xs text-muted-foreground">
                {item.daysUntilStart < 7 ? (
                  <span className="flex items-center gap-1 text-chart-4">
                    <AlertCircle className="h-3 w-3" />
                    Starting soon
                  </span>
                ) : item.daysUntilStart < 30 ? (
                  <span className="flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    This month
                  </span>
                ) : item.daysUntilStart < 90 ? (
                  <span className="flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    This quarter
                  </span>
        ...
        case "completionDate":
          return (
            <div className="space-y-1">
              <div className="font-medium text-base">{item.completionDate}</div>
              <div className="text-xs text-muted-foreground">{item.duration}</div>
            </div>
          )
          ...
          return (
            <div className="space-y-1">
              ...
              <div className="text-xs text-muted-foreground flex items-center gap-1">
                {parseFloat(item.finalEvaluation) >= 4.7 ? (
                  <>
                  ...
                  <>
                    ...
                    <span className="text-chart-1">Good</span>
                  </>
                )}
              </div>
            </div>
        ...
        case "actions": {
          ...
          return (
            <div className="flex items-center gap-1">
              <Button variant="outline" size="sm" className="h-7 px-2" onClick={primaryAction.onClick}>
  ...
  // ── Banner card for the metrics section ────────────────────────────────
