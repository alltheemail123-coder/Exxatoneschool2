...
const TAB_ICONS: Record<string, React.ReactNode> = {
  ...
  "pending-review": <Eye className="h-4 w-4" />,
...
const requestedColumns: ColumnConfig[] = autoSuggestColumnPinning([
  ...
  { key: "discipline", label: "Discipline/Specialization", icon: GraduationCap, isPinned: false, isVisible: true, width: 220, minWidth: 180, sortable: true, filterable: true, groupable: true },
...
const bulkActionDefs = [
  { label: "Approve Selected", icon: CheckCircle, action: "approve", variant: "default" as const },
  { label: "Reject Selected", icon: XCircle, action: "reject", variant: "default" as const },
]
...
export function SlotsPage({ onItemClick }: SlotsPageProps) {
  ...
  // React 18: Deferred search value — keeps input responsive while table filters in background
  ...
  // ── Data subsets for 3 tabs ───────────────────────────────────────────
  const requestedData = React.useMemo(
    () => availabilityData.filter((item) => item.isNewRequest),
    []
  )
  const pendingReviewData = React.useMemo(
    () => availabilityData.filter((item) => !item.isNewRequest && item.hasRecentRequest),
    []
  )
  const confirmedData = React.useMemo(
    () => availabilityData.filter((item) => !item.isNewRequest && !item.hasRecentRequest),
    []
  )
  ...
  const computedMetrics = React.useMemo(() => {
    ...
    const confirmationRate = confirmedData.length > 0
      ? Math.round((confirmedData.length / totalSlots) * 100)
      : 0

    return [
      createSimpleMetricData("Total Slots", String(totalSlots), {
        trend: "up",
        trendValue: "+18",
      }),
      createSimpleMetricData("New Requests", String(pendingRequests), {
        trend: pendingRequests > 10 ? "up" : "down",
        trendValue: pendingRequests > 10 ? `+${pendingRequests - 10}` : `-${10 - pendingRequests}`,
      }),
      createSimpleMetricData("Pending Review", String(pendingReviews), {
        trend: pendingReviews > 10 ? "up" : "down",
        trendValue: pendingReviews > 10 ? `+${pendingReviews - 10}` : `-${10 - pendingReviews}`,
      }),
      createSimpleMetricData("Confirmation Rate", `${confirmationRate}%`, {
        trend: confirmationRate >= 60 ? "up" : "down",
        trendValue: confirmationRate >= 60 ? "+3" : "-2",
      }),
    ]
  ...
  const filteredData = React.useMemo(() => {
    ...
    tableFilters.forEach((filter) => {
      if (filter.values.length > 0) {
        data = data.filter((item) =>
          filter.values.includes(item[filter.columnKey as keyof typeof item] as string)
        )
      }
    })
    ...
    if (tableSorts.length > 0) {
      data = [...data].sort((a, b) => {
        for (const sort of tableSorts) {
          const aVal = a[sort.columnKey as keyof typeof a]
          const bVal = b[sort.columnKey as keyof typeof b]
          let cmp = 0
          if (aVal! < bVal!) cmp = -1
          if (aVal! > bVal!) cmp = 1
    ...
    () => [
      {
        ...
        label: "Location",
        icon: MapPin,
        options: Array.from(new Set(availabilityData.map((i) => i.location))).sort(),
      },
      {
        key: "discipline",
        label: "Discipline",
        icon: GraduationCap,
        options: Array.from(new Set(availabilityData.map((i) => i.discipline))).sort(),
      },
      {
        key: "experienceType",
        label: "Experience Type",
        icon: Users,
        options: Array.from(new Set(availabilityData.map((i) => i.experienceType))).sort(),
      },
    ],
    []
  )
  ...
  const handleMetricClick = React.useCallback(
    (index: number) => {
      // 0 = Total Slots, 1 = New Requests → requested, 2 = Pending Review → pending-review, 3 = Confirmation Rate → confirmed
      if (index === 2) {
  ...
  const metricsBanner = showBanner ? (
    <InsightCard
      data={createInsightCardData(
        ...
        `${pendingReviewData.length} slots are pending review with an average wait of 4.2 days. Clearing these could improve confirmation rate by ~8%.`,
        Sparkles
      )}
      onClick={() => {
        setActiveTab("pending-review")
  ...
  const renderCell = React.useCallback(
    (column: ColumnConfig, item: any, _index: number) => {
      // Helper to get text class based on wrapText setting
      ...
      if (isRequestedTab) {
        switch (column.key) {
            ...
            return (
              <div className="space-y-1 w-full overflow-hidden">
                ...
                <div className={`flex items-center gap-2 text-xs text-muted-foreground ${getTextClass()}`}>
                  ...
                  {item.requestedBy === "school" && item.program && (
                    <>
                      <span>•</span>
                      <span>{item.program} Program</span>
                    </>
                  )}
                </div>
              </div>
          ...
          case "shiftAndDays":
            return (
              <div className="space-y-1 w-full overflow-hidden">
                <div className={getTextClass("font-medium text-base")}>Evening (13:00 - 17:00)</div>
                <div className={getTextClass("text-xs text-muted-foreground")}>Mon, Wed, Fri</div>
              </div>
            )
          case "requestedDuration":
            return (
              <div className="space-y-1 w-full overflow-hidden">
                <div className={getTextClass("font-medium text-base")}>
          ...
          case "slots":
            return (
              <div className="space-y-1">
                <div className="font-medium text-base">{item.totalRequest || 5} Requested</div>
                <div className="text-xs text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <CheckCircle className="h-3 w-3 text-chart-2" />
                    {item.pendingReview || 2} confirmed
                  </span>
                </div>
              </div>
            )
          ...
          case "status":
            return (
              <Badge
                variant={item.isNewRequest ? "default" : "secondary"}
                className={item.isNewRequest ? "bg-chart-4/10 text-chart-4 border-chart-4/20" : "bg-chart-2/10 text-chart-2 border-chart-2/20"}
              >
                {item.isNewRequest ? "Request Pending" : "Approved"}
              </Badge>
            )
          case "actions":
            return (
          ...
          case "name":
            return (
              <div className="space-y-1 w-full overflow-hidden">
                ...
                <div className={getTextClass("text-xs text-muted-foreground")}>ID: {item.id}</div>
              </div>
            )
          case "schedule":
            return (
            ...
            return (
              <div className="space-y-1">
                <div className="font-medium text-base">
                  {item.totalRequest}/{item.totalSlots || 10} Assigned
                </div>
                <div className="text-xs text-muted-foreground">
                  {item.totalSlots && item.totalSlots - item.totalRequest > 0 ? (
                    <span className="flex items-center gap-1">
                      <AlertCircle className="h-3 w-3 text-chart-4" />
                      {item.totalSlots - item.totalRequest} to assign
                    </span>
                  ) : (
                    <span className="flex items-center gap-1">
                      <CheckCircle className="h-3 w-3 text-chart-2" />
            ...
            return (
              <div>
                {item.lastRequestTime ? (
                  <div className="flex items-center gap-1">
                    <CheckCircle className="h-3 w-3 text-chart-2" />
                    <span className="text-foreground font-medium text-base">{item.lastRequestTime}</span>
                  </div>
                ) : (
      ...
      switch (column.key) {
        case "experienceType":
          return (
            <Badge variant="secondary" withIcon iconPosition="left" className="w-fit">
              {item.experienceType === "Group" ? <Users className="h-3 w-3" /> : <User className="h-3 w-3" />}
              {item.experienceType}
            </Badge>
          )
        case "location":
        ...
        case "discipline":
          return (
            <div className="space-y-1 w-full overflow-hidden">
              <div className={getTextClass("font-medium text-base")}>{item.discipline}</div>
              <div className={getTextClass("text-xs text-muted-foreground")}>{item.specialization}</div>
            </div>
          )
        default:
  ...
  return (
    <PrimaryPageTemplate
      title="Slots"
      description="Track and manage slot requests, approvals, and assignments"
      metrics={{
