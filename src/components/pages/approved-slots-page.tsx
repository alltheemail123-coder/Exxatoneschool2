...
import {
  Activity, Filter, TrendingUp, UserCheck, Edit, Eye, Trash2, Sparkles,
  CheckCircle, Users, User, MapPin, Calendar, Clock, AlertCircle, DoorOpen,
  Bell, MoreHorizontal, Copy, Archive, Settings, GraduationCap, Table2,
  Kanban, ChevronDown, XCircle,
} from "lucide-react"
...
const approvedSlotsMetrics = [
  ...
  createMetricCardData("Active Slots", "98", Activity, "text-chart-1", { change: "+12", trend: "up", description: "Currently active" }),
...
export function ApprovedSlotsPage({ onItemClick }: ApprovedSlotsPageProps) {
  ...
  const [columns, setColumns] = React.useState<ColumnConfig[]>(() => [
    ...
    { key: "name", label: "Approved Slot", icon: DoorOpen, isPinned: false, pinSide: undefined, isVisible: true, width: 300, minWidth: 200, sortable: true, filterable: true, groupable: true, options: Array.from(new Set(availabilityData.map(item => item.name))).sort() },
  ...
  const handleAddFilter = (filterKey: string) => { const existing = activeFilters.find(f => f.key === filterKey); if (existing) { setShowFilters(true); return; } setActiveFilters(prev => [...prev, { id: `${filterKey}_${Date.now()}`, key: filterKey, label: filterConfigs.find(c => c.key === filterKey)?.label || filterKey, values: [] }]); setShowFilters(true) }
  ...
  const renderCell = (column: ColumnConfig, item: any, index: number) => {
    switch (column.key) {
      case "name":
        return (<div className="space-y-1 w-full overflow-hidden"><div className="flex items-center gap-2 min-w-0"><div className="line-clamp-2 data-table-clickable min-w-0 flex-1 break-words font-medium text-base" onClick={() => onItemClick?.(item.id)}>{item.name}</div></div><div className="text-xs text-muted-foreground truncate">ID: {item.id}</div></div>)
      case "experienceType":
  ...
  return (
    <div className="approved-slots-page-container px-4 lg:px-6 pt-4 lg:pt-6 pb-20 space-y-6 max-w-full overflow-clip" data-page="approved-slots">
      <div className="space-y-2"><h1 className="text-2xl font-bold text-foreground font-display">Approved Slots</h1><p className="text-muted-foreground">Manage and monitor approved slot assignments for students</p></div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-clip min-w-0">
        <div className="flex items-center justify-between p-4 border-b border-border">
          ...
          <div className="flex items-center gap-4">
            <OutlineSearchInput placeholder="Search approved slots..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-64" />
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="upcoming" className="min-h-[400px] m-0"><div className="p-8 text-center text-muted-foreground h-full flex flex-col items-center justify-center"><Calendar className="h-12 w-12 mx-auto mb-4 text-muted-foreground/30" /><p>No upcoming approved slots</p></div></TabsContent>
