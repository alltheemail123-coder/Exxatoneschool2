...
import {
  Activity, AlertCircle, Archive, Calendar, CheckCircle, Clock, Edit, Eye, GraduationCap,
  Heart, ListChecks, MapPin, MoreHorizontal, Percent, Sparkles, Star, User, Users, XCircle,
} from "lucide-react"
...
const TAB_ICONS: Record<string, React.ReactNode> = { open: <Heart className="h-4 w-4" />, closed: <Archive className="h-4 w-4" /> }
...
export function WishlistPage({ onItemClick }: WishlistPageProps) {
  ...
  const filterConfigs: FilterConfig[] = React.useMemo(() => [
    { key: "specialization", label: "Specialization", icon: GraduationCap, options: Array.from(new Set(wishlistData.map((i) => i.specialization))).sort() },
  ...
  const handleAddFilter = React.useCallback((filterKey: string) => {
    ...
    setActiveFilters((prev) => [...prev, { id: `${filterKey}_${Date.now()}`, key: filterKey, label: filterConfigs.find((c) => c.key === filterKey)?.label || filterKey, values: [] }]); setShowFilters(true)
  ...
  const bulkActions: PrimaryPageBulkAction[] = React.useMemo(() => bulkActionDefs.map((a) => ({
    label: a.label, icon: a.icon, onClick: () => { console.log(`Bulk: ${a.action}`, selectedItems); setSelectedItems([]) }, variant: a.variant,
  })), [selectedItems])
  ...
  const getStatusBadge = React.useCallback((status: WishlistItem["status"]) => {
    switch (status) {
      case "Collecting": return <Badge variant="secondary" className="bg-chart-1/10 text-chart-1 border-chart-1/20"><ListChecks className="h-3 w-3 mr-1" />Collecting</Badge>
      case "In Review": return <Badge variant="secondary" className="bg-chart-4/10 text-chart-4 border-chart-4/20"><Eye className="h-3 w-3 mr-1" />In Review</Badge>
      case "Matching": return <Badge variant="secondary" className="bg-chart-5/10 text-chart-5 border-chart-5/20"><Activity className="h-3 w-3 mr-1" />Matching</Badge>
      case "Completed": return <Badge variant="secondary" className="bg-chart-2/10 text-chart-2 border-chart-2/20"><CheckCircle className="h-3 w-3 mr-1" />Completed</Badge>
      case "Closed": return <Badge variant="secondary" className="bg-muted text-muted-foreground border-border"><Archive className="h-3 w-3 mr-1" />Closed</Badge>
    }
  }, [])

  const getPriorityBadge = React.useCallback((priority: WishlistItem["priority"]) => {
    switch (priority) {
      case "High": return <Badge variant="secondary" className="bg-destructive/10 text-destructive border-destructive/20">High</Badge>
      case "Medium": return <Badge variant="secondary" className="bg-chart-4/10 text-chart-4 border-chart-4/20">Medium</Badge>
  ...
  const renderCell = React.useCallback(
    (column: ColumnConfig, item: WishlistItem, _index: number) => {
      const getTextClass = (baseClass: string = "") => column.wrapText ? `${baseClass} break-words` : `${baseClass} truncate`
      switch (column.key) {
        case "name":
          return (<div className="space-y-1 w-full overflow-hidden"><div className="flex items-center gap-2 min-w-0"><div className={`line-clamp-2 data-table-clickable min-w-0 flex-1 font-medium text-base ${column.wrapText ? "break-words" : ""}`} onClick={() => onItemClick?.(item.id)}>{item.name}</div></div><div className={`flex items-center gap-2 text-xs text-muted-foreground ${getTextClass()}`}><span>{item.id}</span><span>&middot;</span><span>{item.topSites.slice(0, 2).join(", ")}{item.topSites.length > 2 ? ` +${item.topSites.length - 2}` : ""}</span></div></div>)
        case "specialization":
          return <div className={column.wrapText ? "font-medium text-base break-words" : "font-medium text-base truncate"}>{item.specialization}</div>
        case "rotationPeriod":
        ...
        case "students":
          return (<div className="space-y-1"><div className="font-medium text-base">{item.preferencesSubmitted}/{item.totalStudents} Submitted</div><div className="text-xs text-muted-foreground">{item.preferencesSubmitted < item.totalStudents ? (<span className="flex items-center gap-1"><Clock className="h-3 w-3 text-chart-4" />{item.totalStudents - item.preferencesSubmitted} pending</span>) : (<span className="flex items-center gap-1"><CheckCircle className="h-3 w-3 text-chart-2" />All submitted</span>)}</div></div>)
        case "matchRate":
          return (<div className="space-y-1"><div className="font-medium text-base">{item.matchRate > 0 ? `${item.matchRate}%` : "—"}</div>{item.matchRate > 0 && (<div className="text-xs text-muted-foreground">{item.matchRate >= 85 ? (<span className="flex items-center gap-1 text-chart-2"><CheckCircle className="h-3 w-3" />Strong</span>) : item.matchRate >= 60 ? (<span className="flex items-center gap-1 text-chart-4"><AlertCircle className="h-3 w-3" />Moderate</span>) : (<span className="flex items-center gap-1 text-destructive"><AlertCircle className="h-3 w-3" />Low</span>)}</div>)}</div>)
        case "priority":
        ...
        case "actions":
          return (<DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="sm" className="h-7 w-7 p-0"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48"><DropdownMenuItem onClick={() => onItemClick?.(item.id)}><Eye className="h-4 w-4 mr-2" />View Details</DropdownMenuItem><DropdownMenuItem><Edit className="h-4 w-4 mr-2" />Edit</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem className="text-destructive"><XCircle className="h-4 w-4 mr-2" />Close Wishlist</DropdownMenuItem></DropdownMenuContent></DropdownMenu>)
        default:
