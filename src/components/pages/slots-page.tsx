...
export function SlotsPage({ onItemClick }: SlotsPageProps) {
  ...
  const filterConfigs: FilterConfig[] = React.useMemo(() => [
    ...
    { key: "discipline", label: "Discipline", icon: GraduationCap, options: Array.from(new Set(availabilityData.map((i) => i.discipline))).sort() },
  ...
  const bulkActions: PrimaryPageBulkAction[] = React.useMemo(() => bulkActionDefs.map((a) => ({
    ...
    onClick: () => { console.log(`Bulk: ${a.action}`, selectedItems); setSelectedItems([]) },
    variant: a.variant,
  })), [selectedItems])
  ...
  const renderCell = React.useCallback(
    (column: ColumnConfig, item: any, _index: number) => {
      ...
      if (isRequestedTab) {
        switch (column.key) {
          case "siteName":
            return (<div className="space-y-1 w-full overflow-hidden"><div className="flex items-center gap-2 min-w-0"><div className={`line-clamp-2 data-table-clickable min-w-0 flex-1 font-medium text-base ${column.wrapText ? 'break-words' : ''}`} onClick={() => onItemClick?.(item.id)}>{item.location}</div></div><div className={`flex items-center gap-2 text-xs text-muted-foreground ${getTextClass()}`}><span>Request ID: {item.id}</span>{item.requestedBy === "school" && item.program && (<><span>•</span><span>{item.program} Program</span></>)}</div></div>)
          case "shiftAndDays":
          ...
          case "status":
            return (<Badge variant={item.isNewRequest ? "default" : "secondary"} className={item.isNewRequest ? "bg-chart-4/10 text-chart-4 border-chart-4/20" : "bg-chart-2/10 text-chart-2 border-chart-2/20"}>{item.isNewRequest ? "Request Pending" : "Approved"}</Badge>)
          case "actions":
            return (<div className="flex items-center gap-1"><Button variant="ghost" size="sm" className="h-7 w-7 p-0"><Edit className="h-4 w-4" /></Button><Button variant="ghost" size="sm" className="h-7 w-7 p-0"><MoreHorizontal className="h-4 w-4" /></Button></div>)
        }
      }
      if (!isRequestedTab) {
        switch (column.key) {
          case "name":
            return (<div className="space-y-1 w-full overflow-hidden"><div className="flex items-center gap-2 min-w-0"><div className={`line-clamp-2 data-table-clickable min-w-0 flex-1 font-medium text-base ${column.wrapText ? 'break-words' : ''}`} onClick={() => onItemClick?.(item.id)}>{item.name}</div></div><div className={getTextClass("text-xs text-muted-foreground")}>ID: {item.id}</div></div>)
          case "schedule":
