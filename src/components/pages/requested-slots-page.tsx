...
export function RequestedSlotsPage({ onItemClick }: RequestedSlotsPageProps) {
  ...
  const [columns, setColumns] = React.useState<ColumnConfig[]>(() => {
    const defaultColumns: ColumnConfig[] = [
      ...
      { key: "discipline", label: "Discipline/Specialization", icon: GraduationCap, isPinned: false, pinSide: undefined, isVisible: true, width: 220, minWidth: 180, sortable: true, filterable: true, groupable: true },
  ...
  const handleAddFilter = (filterKey: string) => {
    ...
    const newFilter: ActiveFilter = { id: `${filterKey}_${Date.now()}`, key: filterKey, label: filterConfigs.find(config => config.key === filterKey)?.label || filterKey, values: [] }
  ...
  const renderCell = (column: ColumnConfig, item: any, index: number) => {
    switch (column.key) {
        ...
        return (
          <div className="space-y-1 w-full overflow-hidden">
            <div className="flex items-center gap-2 min-w-0">
              <div className="line-clamp-2 data-table-clickable min-w-0 flex-1 break-words font-medium text-base" onClick={() => onItemClick?.(item.id)}>{item.location}</div>
            </div>
      ...
      case "actions":
        return (<div className="flex items-center gap-1"><Button variant="ghost" size="sm" className="h-7 w-7 p-0"><Edit className="h-4 w-4" /><span className="sr-only">Edit</span></Button><Button variant="ghost" size="sm" className="h-7 w-7 p-0"><MoreHorizontal className="h-4 w-4" /><span className="sr-only">More actions</span></Button></div>)
      default:
  ...
  return (
    <div className="requested-slots-page-container px-4 lg:px-6 pt-4 lg:pt-6 space-y-6 max-w-full overflow-clip" data-page="requested-slots">
      <div className="space-y-2">
        <h1 className="text-2xl font-bold text-foreground font-display">Requested Slots</h1>
      ...
      <div className="bg-card border border-border rounded-lg overflow-clip min-w-0">
        <div className="flex items-center justify-between p-4 border-b border-border">
          ...
          <div className="flex items-center gap-4">
            <OutlineSearchInput placeholder="Search requests..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-64" />
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="urgent" className="min-h-[400px] m-0"><div className="p-8 text-center text-muted-foreground h-full flex flex-col items-center justify-center"><Clock className="h-12 w-12 mx-auto mb-4 text-muted-foreground/30" /><p>No urgent requests found</p></div></TabsContent>
          <TabsContent value="this-week" className="min-h-[400px] m-0"><div className="p-8 text-center text-muted-foreground h-full flex flex-col items-center justify-center"><Calendar className="h-12 w-12 mx-auto mb-4 text-muted-foreground/30" /><p>No requests from this week</p></div></TabsContent>
        </Tabs>
      </div>
